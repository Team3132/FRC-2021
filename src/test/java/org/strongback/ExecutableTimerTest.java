...
/**
 * This test attempts to measure the timing of the {@link Strongback#executor() Strongback Executor}
 * instance at various periods
 * <p>
 * The code committed into Git are small execution rates and sample sizes to minimize the time
 * required to run the tests. As
 * such, the results are probably not terribly meaningful. To obtain more meaningful results on your
 * own platform, simply adjust
...
public class ExecutableTimerTest {
    ...
    protected void runTimer(int millisecondExecutionPeriod, int sampleTimeInMilliseconds) {
        try {
            ...
            // (Previously was wrapped in assertThat that didn't actually assert true or false.)
            ExecutableTimer.measureTimingAndPrint(Strongback.executor(),
                    " for " + millisecondExecutionPeriod + " ms",
                    sampleTimeInMilliseconds / millisecondExecutionPeriod)
                    .await(4, TimeUnit.SECONDS).isComplete();
