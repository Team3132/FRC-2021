...
class FirstPython:
    ...
    def parseSerial(self, str):
        ...
        try:
            ...
            if parts[0] == "position":
                if len(parts) != 4:
                    raise Exception("Insufficient number of parameters for camera position, expected four")
                self.cameraHeight = int(parts[1])
                self.cameraPitch = int(parts[2])
                self.img_rotate_angle = int(parts[3])
                return "Camera position set to: Height: {}metres, Pitch: {}degrees, Roll: {}degrees".format(self.cameraHeight, self.cameraPitch, self.img_rotate_angle)
    ...
    def detect(self, imgbgr, outimg = None):
        ...
        try:
            ...
            center_direct_distance = cal_goal_center_direct_distance(distance, self.cameraHeight)
            skew = cal_goal_skew(TL,TR,BL,BR, center_direct_distance, self.cameraPitch, self.cameraHeight)
            angle = cal_angle(mx)
