...
/**
 * A command that measures the
 * {@link Strongback.Configurator#useExecutionPeriod(long, java.util.concurrent.TimeUnit) execution
...
public final class ExecutableTimer implements Executable {
    ...
    /**
     * Measure the timing of the {@link Executor} and print the histogram of timing results to
     * {@link System#out}. The resulting
     * timer will be {@link Executor#register(Executable) registered} with the Executor, and upon
     * completion will automatically
     * unregister itself.
     * <p>
     * This method can be used to print a histogram of {@link Strongback#executor() Strongback's
     * executor}. The following
     * example uses 1000 samples, which corresponds to 5 seconds if the
     * {@link Configurator#useExecutionPeriod(long, TimeUnit)
     ...
     */
    public static ExecutableTimer measureTimingAndPrint(Executor executor, String desc,
            int numberOfSamples) {
        return measureTiming(executor, numberOfSamples, snapshot -> {
            System.out.println(
                    "Execution timing statistics" + (desc != null ? " (" + desc + ")" : ""));
    ...
    public static ExecutableTimer measureTiming(Executor executor, int numberOfSamples,
            Consumer<Snapshot> atCompletion) {
