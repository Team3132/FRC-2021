...
public class Controller implements Runnable, DashboardUpdater, LogHelper {
    ...
    // FIXME: Mark thinks that we should have a new mode for the colour wheel.
    // Details: Copy the existing POSITION type to make a FMS_POSITION that
    // gets the colour from the FMS inside ColourWheelImpl and then
    // the controller doesn't need to care about the mode. The colourwheel
    // will need to know what the FMS is asking for.
    private Supplier<LEDColour> allianceColour;
    ...
    public Controller(Subsystems subsystems, Supplier<WheelColour> fmsColour,
            Supplier<LEDColour> allianceColour) {
