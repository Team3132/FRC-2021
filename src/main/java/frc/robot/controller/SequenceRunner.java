...
public class SequenceRunner implements LogHelper {
    ...
    synchronized public boolean next() {
        ...
        if (status == RunStatus.ABORTED) {
            // The end state has been fully applied. Move to the finished.
            status = RunStatus.FINISHED;
            return false;
        }
    ...
    synchronized public void abort() {
        if (status == RunStatus.FINISHED || status == RunStatus.ABORTING) {
            // Already finished or aborted.
            return;
        }
        if (status == RunStatus.WAITING_TO_START) {
            // Give up on this sequence, it never got to start.
            status = RunStatus.FINISHED;
            return;
        }
     ...
     */
    synchronized public boolean tryApplyState() throws Exception {
        boolean isFirstApplyState = firstApplyState;
        firstApplyState = false;
        // Check that it's running or current isn't valid.
        if (status == RunStatus.FINISHED) {
            blockedBy = "";
            return true;
        }
        if (status != RunStatus.RUNNING && status != RunStatus.ABORTING
                && status != RunStatus.ABORTED) {
            throw new Exception(
                    "SequenceRunner(" + sequence.getName() + ") is in unexpected state "
                            + status.name());
        }
        ...
        // First we tell the subystems what they should do so they can do it in parallel before
        ...
        if (desiredState.climberRatchet != null) {
            subsystems.drivebase.applyRatchet(desiredState.climberRatchet);
        }
        ...
        // Loader
        if (desiredState.loaderSpinnerRPS != null) {
            subsystems.loader.setTargetSpinnerRPS(desiredState.loaderSpinnerRPS);
        }
        if (desiredState.loaderPassthroughDutyCycle != null) {
            subsystems.loader
                    .setTargetPassthroughDutyCycle(desiredState.loaderPassthroughDutyCycle);
        }
        if (desiredState.loaderBlocker != null) {
            subsystems.loader.setBlocker(desiredState.loaderBlocker);
        }

        // Colour wheel
        if (desiredState.colourArmPosition != null) {
            subsystems.colourWheel.setArmPosition(desiredState.colourArmPosition);
        }
        if (desiredState.colourAction != null) {
        ...
        if (desiredState.shooterHoodPosition != null) {
            subsystems.shooter.setHoodPosition(desiredState.shooterHoodPosition);
        }
        ...
        // Zipchain/climber
        if (desiredState.zipChainDutyCycle != null) {
            subsystems.zipChain.setDutyCycle(desiredState.zipChainDutyCycle);
        }
        if (desiredState.climberDeployerDutyCycle != null) {
            subsystems.climberDeployer.setDutyCycle(desiredState.climberDeployerDutyCycle);
        }
        if (desiredState.climberHolder != null) {
            subsystems.climberDeployer.setHolder(desiredState.climberHolder);
        }
        if (desiredState.buddyClimb != null) {
            subsystems.buddyClimb.setEnabled(desiredState.buddyClimb);
        }

        if (desiredState.climbMode != null) {
            subsystems.drivebase.setClimbMode(desiredState.climbMode);
        }
        ...
        // Has got the correct number of balls on board?
        if (notFinished(desiredState.expectedNumberOfBalls,
                hasEnoughBalls(desiredState.expectedNumberOfBalls,
                        subsystems.loader.getCurrentBallCount()),
                LEDColour.MAGENTA, "ball count")) {
            return false;
        }

        // Buddy climb.
        if (notFinished(desiredState.buddyClimb, subsystems.buddyClimb.isInPosition(),
                LEDColour.BROWN, "buddy climb")) {
        ...
        // Loader blocker
        if (notFinished(desiredState.loaderBlocker, subsystems.loader.blockerIsInPosition(),
                LEDColour.BLUE, "paddle")) {
            return false; // It's not yet in position
        }

        // Shooter hood
        if (notFinished(desiredState.shooterHoodPosition, subsystems.shooter.hoodIsInPosition(),
                LEDColour.GREEN, "hood")) {
            return false; // It's not yet in position
        }

        // Shooter speed
        if (notFinished(desiredState.shooterUpToSpeed, subsystems.shooter.isAtTargetSpeed(),
                LEDColour.CYAN, "shooter wheel")) {
            return false;
        }

        // Colour wheel
        if (notFinished(desiredState.colourArmPosition, subsystems.colourWheel.armIsInPosition(),
                LEDColour.ORANGE, "colour wheel")) {
            return false; // It's not yet in position
        }

        if (notFinished(desiredState.colourAction, subsystems.colourWheel.isFinished(),
                LEDColour.ORANGE, "colour wheel")) {
            return false; // Colour wheel is moving.
        }

        if (notFinished(desiredState.drive, subsystems.drivebase.hasFinished(),
                LEDColour.RED, "driving")) {
    ...
    private boolean hasEnoughBalls(Integer ballCount, int actualBallCount) {
        if (ballCount == null) {
            // No expectation on the number of balls.
            return true;
        }
        return actualBallCount == ballCount.intValue();
    }
