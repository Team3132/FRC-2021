...
public class DrivebaseImpl extends Subsystem implements Drivebase {
    ...
    private FeatureSolenoid ptoSolenoid;
    private FeatureSolenoid ratchetSolenoid;
    private DriveMotion currentMotion;

    public DrivebaseImpl(Motor left, Motor right, Solenoid ptoSolenoid, Solenoid ratchetSolenoid) {
        super("Drive");
        this.left = left;
        this.right = right;
        this.ptoSolenoid = new FeatureSolenoid(ptoSolenoid);
        this.ratchetSolenoid = new FeatureSolenoid(ratchetSolenoid);
        ...
        Chart.register(() -> isRatchetApplied(), "%s/extended", name);
        Chart.register(() -> isRatchetReleased(), "%s/retracted", name);
    }

    @Override
    public Drivebase setClimbMode(Mode mode) {
        ptoSolenoid.setMode(mode);
        return this;
    }

    @Override
    public Drivebase applyRatchet(Mode extend) {
        ratchetSolenoid.setMode(extend);
        return this;
    }
    ...
    public boolean isClimbModeEnabled() {
        ...
        return ptoSolenoid.isEnabled();
    }

    @Override
    public boolean isDriveModeEnabled() {
        // Logger.debug("Is intake extended: " + solenoid.isExtended());
        return ptoSolenoid.isDisabled();
    }

    @Override
    public boolean isRatchetApplied() {
        // Logger.debug("Is intake extended: " + solenoid.isExtended());
        return ratchetSolenoid.isEnabled();
    }

    @Override
    public boolean isRatchetReleased() {
        // Logger.debug("Is intake extended: " + solenoid.isExtended());
        return ratchetSolenoid.isDisabled();
    }
