...
public class OIImpl {
    ...
    public void configureDriverJoysticks(FlightStick left, FlightStick right) {
        ...
        // Release/enable ratchets
        left.button(4).onPress(run(Sequences.releaseClimberRatchet()));
        left.button(4).onRelease(run(Sequences.applyClimberRatchet()));

        // Enable drive mode
        left.button(5).onPress(run(Sequences.enableClimbMode()));

        // Enable climb
        left.button(3).onPress(run(Sequences.setDrivebaseToDefault()));

        // Vision lineup until the driver releases the button.
        ...
        // Reverse intake
        right.button(4).onPress(run(Sequences.reverseIntaking()));
        right.button(4).onRelease(run(Sequences.stopIntaking()));

        // Reverse loader and intake
        right.button(6).onPress(run(Sequences.reverseIntakingAndLoader()));
        right.button(6).onRelease(run(Sequences.stopIntaking()));
    }
    ...
    public void configureOperatorJoystick(Gamepad gamepad) {
        // Close/ender shot
        gamepad.leftTrigger()
                .onPress(run(Sequences.spinUpCloseShot(Config.shooter.speed.closeRPS)));
        // Auto line/front trench shot
        gamepad.leftBumper()
                .onPress(run(Sequences.spinUpFarShot(Config.shooter.speed.autoLineRPS)));
        // Colourwheel/back trench shot
        gamepad.rightBumper()
                .onPress(run(Sequences.spinUpFarShot(Config.shooter.speed.farTargetRPS)));

        // Acutally shoot the balls
        // YOU MUST use one of the spinUp buttons before pressing this
        gamepad.rightTrigger().onPress(run(Sequences.startShooting()));
        gamepad.rightTrigger().onRelease(run(Sequences.stopShooting()));

        // Stow the intake
        gamepad.backButton().onPress(run(Sequences.raiseIntake()));

        // Extend climber deployer
        gamepad.leftAxis().north().onPress(run(Sequences.extendClimberDeployer()));
        gamepad.leftAxis().north().onRelease(run(Sequences.stopClimberDeployer()));

        // Retract climber deployer
        gamepad.leftAxis().south().onPress(run(Sequences.retractClimberDeployer()));
        gamepad.leftAxis().south().onRelease(run(Sequences.stopClimberDeployer()));

        // Release climber deployer
        gamepad.aButton().onPress(run(Sequences.releaseClimberDeployer()));

        // Stow the climber deployer
        gamepad.startButton().onPress(run(Sequences.stowClimberDeployer()));

        // Intake
        gamepad.xButton().onPress(run(Sequences.startIntaking()));
        gamepad.xButton().onRelease(run(Sequences.stopIntaking()));

        // Reverse intake
        gamepad.yButton().onPress(run(Sequences.reverseIntaking()));
        gamepad.yButton().onRelease(run(Sequences.stopIntaking()));

        // Reverse loader and intake
        gamepad.bButton().onPress(run(Sequences.reverseIntakingAndLoader()));
        gamepad.bButton().onRelease(run(Sequences.stopIntaking()));
    }
    ...
    public void configureDDRPad(Dancepad dancepad) {
        ...
        // Close/fender shot
        dancepad.selectButton()
                .onPress(run(Sequences.spinUpCloseShot(Config.shooter.speed.closeRPS)));

        // Auto line/front trench shot
        dancepad.R2Button().onPress(run(Sequences.spinUpFarShot(Config.shooter.speed.autoLineRPS)));
    ...
    public void configureDiagBox(DiagnosticBox box) {
        ...
        // turn off.
        box.redButton(2).onPress(() -> shooterIF.setHoodPosition(Position.EXTENDED));
        box.redButton(3).onPress(() -> shooterIF.setHoodPosition(Position.RETRACTED));
        box.redButton(1).whileTriggered(() -> {
            shooterIF.setTargetRPS(1.5 * Config.shooter.speed.closeRPS * box.getRedPot().read());
        });
        ...
        // Loader overrides.
        OverridableSubsystem<Loader> loaderOverride = subsystems.loaderOverride;
        // Get the interface that the diag box uses.
        Loader loaderIF = loaderOverride.getOverrideInterface();
        // Setup the switch for manual/auto/off modes.
        mapOverrideSwitch(box, DiagnosticBox.Colour.GREEN, loaderOverride);
        // While the loader speed button is pressed, set the target speed. Does not turn
        // off.
        box.greenButton(1).whileTriggered(() -> loaderIF
                .setTargetSpinnerRPS(
                        1.5 * Config.loader.spinner.speed.intakingRPS * box.getGreenPot().read()));
        box.greenButton(1).onRelease(() -> loaderIF.setTargetSpinnerRPS(0));
        // Passthrough.
        box.blueButton(1).whileTriggered(
                () -> loaderIF.setTargetPassthroughDutyCycle(box.getBluePot().read()));
        box.blueButton(1).onRelease(() -> loaderIF.setTargetPassthroughDutyCycle(0));
        // Paddle / blocker.
        box.greenButton(3).onPress(() -> loaderIF.setBlocker(Mode.ENABLED));
        box.greenButton(2).onPress(() -> loaderIF.setBlocker(Mode.DISABLED));
    }
    ...
    public void registerDriveRoutines(FlightStick left, FlightStick right,
        ...
        drivebase.registerDriveRoutine(DriveRoutineType.VISION_ASSIST,
                new VisionAssist(
                        () -> left.getAxis(1).invert().squarePreservingSign().read(),
                        () -> right.getAxis(0).squarePreservingSign().read(),
     ...
     * <pre>
     * {@code
     * onMode(rightStick.getButton(5), rightStick.getButton(6), "climb/drive", Sequences.enableClimbMode(), Sequences.enableDriveMode())
     *   .onPress(rightStick.getButton(1), Sequences.releaseClimberRatchet(), Sequences.startSlowDriveForward())
     *   .onRelease(rightStick.getButton(2), Sequences.releaseClimberRatchet(), Sequences.driveFast());
     * }
     * </pre>
     * 
